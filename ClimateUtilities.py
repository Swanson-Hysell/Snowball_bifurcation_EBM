...
#    *for .addCurve(...) eliminated check to see if input is a list,
#     so that generators can be put in as well and still converted
#     to a numpy array
...
print('Version for debugging readTable')
...
class Curve:
    ...
    def addCurve(self,data,id = '',label = ''):
        ...
        #Transform data  to a numpy array here
...
def scan(buff,inHeader=None,delimiter = None):
    ...
    #     
    startDataLine,endDataLine = findData(buff,delimiter)
    ...
    for line in buff[(startDataLine+istart):endDataLine]:
        ...
        try:
         #The following for loop tests to see if all the items can be converted to float.
         #It is there to catch cases where missing data is replaced by a character string, e.g. ***
         #**ToDo: We should put in some way to handle missing data here, ideally via masked array
...
def findData(buff,delimiter):
    runStarts = []
    for i in range(len(buff)-1):
        if delimiter == None:
            dn = abs(len(buff[i].split())-len(buff[i+1].split()))
        else:
            dn = abs(len(buff[i].split(delimiter))-len(buff[i+1].split(delimiter)))
        if not dn==0:
...
#import string  Eliminated 7/2020 since deprecated
