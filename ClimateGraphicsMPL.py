...
#           DONE 12 May 2020
#    * Clean our leftover Ngl support stuff.
#    * Implement figure saving, via plotObj
#-----------------------------------------------------------------------
...
#                 differing resolution (MPL only)
#     12 May, 2020: Moved subplot creation to get rid of
#          deprecation warning. Also commented out mpl backend
#          and interactive settings. 

print("Using Sandbox version of ClimateGraphicsMPL")

#Try to import matplotlib plotting routines 
try:
    import matplotlib as mpl
    #Configure the backend so things work properly
    #with the Python intepreter. Not needed if you are
    #using ipython. 
    #IDLE is obsolete, but to work properly with idle, idle needs
    ...
    #These options affect whether you get an interactive plot window,
    #or whether the plot is made then frozen
    #mpl.rcParams['backend'] = 'TkAgg'
    #mpl.rcParams['interactive'] = True
    #
    import matplotlib.pyplot as pl #Changed import to pyplot, May 2020
except:
...
#        *There is an annoying MatPlotLib deprecation warning
#         regarding axis re-use. I can't figure out where this
#         is coming from, but we need to get rid of it.
#
...
def plot(*curves):
    ...
    #Create subplot for later manipulation
    #Currently mainly used just to set legend positions
    ax = pl.subplot(111)
    if cMaster.XlogAxis:
        pl.semilogx()
    if cMaster.YlogAxis:
    ...
    #The following block triggers the deprecation warning
    #  but I can't figure out how to fix it. 
    # Seems to work if ax is defined before plotting is done
    # so it has been moved to after pl.figure()
    #ax = pl.subplot(111)
    box = ax.get_position()
    ...
    #The following block is not what triggers the axis deprecation warning
